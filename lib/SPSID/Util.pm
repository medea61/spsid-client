...
sub sync_contained_objects_filtered
{
    my $self = shift;
    my $container = shift;
    my $objclass = shift;
    my $sync_objects = shift;
    my $filter_attribute = shift;
    ...
    my $db_objects = $self->client->search_objects($container, $objclass, $filter_attribute, $filter_value);
    ...
    foreach my $obj ( @add_objects ) {
        my $obj_id = $self->client->create_object($objclass, $obj);

        if(defined $obj->{'siam.device.set_components'}) {
            my $objects = eval { decode_json($obj->{'siam.device.set_components'}) };

            if( $@ )
            {
                print "Cannot decode JSON value in siam.device.set_components condition:" . $@ . "\n";
                next;
            }

            if( ref($objects) ne 'ARRAY' )
            {
                print "The condition value is not a JSON array in siam.device.set_components: " . $obj->{'siam.device.set_components'} . "\n";
                next;
            }

            eval { $self->sync_contained_objects($obj_id, 'SIAM::DeviceComponent', $objects) };
            if( $@ )
            {
                print "Condition siam.device.set_components failed to apply: " . $@ . "\n";
                next;
            }
        }
    }
    ...
    foreach my $id ( keys %modify_objects ) {
        ...
        if(defined $modify_objects{$id}->{'siam.device.set_components'}) {
            my $objects = eval { decode_json($modify_objects{$id}->{'siam.device.set_components'}) };
            ...
            if( ref($objects) ne 'ARRAY' )
            {
                print "The condition value is not a JSON array in siam.device.set_components: " . $modify_objects{$id}->{'siam.device.set_components'} . "\n";
                next;
            }

            eval { $self->sync_contained_objects($id, 'SIAM::DeviceComponent', $objects) };
            if( $@ )
            {
                print "Condition siam.device.set_components failed to apply: " . $@ . "\n";
                next;
